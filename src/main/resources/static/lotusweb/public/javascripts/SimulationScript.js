...
function AlternateSimExec (ev) {
	...
	if (ev.target.id == "btnSimulation"){
		...
		if ( document.getElementById("Simulation").style.display == 'none' ){
			...
			document.getElementById("ProbabReachTab").style.display = 'none';
		}else{
			document.getElementById("Execution").style.display = 'none';
	...
	}else if (ev.target.id == "btnExecution"){
	...
	}else if (ev.target.id == "ProbabReach" || ev.target.id == "ProbabReachCancel" ) {
		
		if ( ev.target.id == "ProbabReach" ){
			verifyStatesProbabReach();
			document.getElementById("Execution").style.display = 'none';
			document.getElementById("Simulation").style.display = 'none';
			document.getElementById("InfoTransition").style.display = 'none';
			document.getElementById("ProbabReachTab").style.display = 'initial';
		}else{
			document.getElementById("Execution").style.display = 'none';
			document.getElementById("Simulation").style.display = 'none';
			document.getElementById("InfoTransition").style.display = 'initial';
			document.getElementById("ProbabReachTab").style.display = 'none';
			document.querySelector("[class*=fa-thumbs]").style.display = 'none';
		}
...
function verifyProbProbabReach (ev) {
	
	if ( ev.target.value > 1 || ev.target.value < 0 ){
		ev.target.value = 0;
	}
