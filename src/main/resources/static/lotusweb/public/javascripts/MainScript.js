...
/* Evento de finalizar a criação de um elemento */
 document.getElementById("RealWorkStation").addEventListener("click", copy_End, false);
...
function clickEventEl (ev) {
	...
	*/
	...
	var obj_state_element = info_manager.state_elements[ev.target.parentElement.id];

	console.log(obj_state_element);
...
function UpdateEventsSvg(data) {

	var svg = document.getElementById("container_svg");

	if ( svg != undefined ){
		svg.innerHTML = data.svg_innerHTML;
	}else{
		/* Caso abra e ainda não exista svg*/
	...
	var g_states = svg.querySelectorAll("g[class=copy_el]");
	var transitions = svg.querySelectorAll("path[id*=linha]");

	var newStateElements = data.infoManager.state_elements;
	var newTransitionsElements = data.infoManager.transitions_elements;

	info_manager.state_elements = {};
	info_manager.transitions_elements = {};

	for ( var i = 0; i < g_states.length; i++ ){

		var gState = g_states[i];
		var state = gState.querySelectorAll("[class=copy_el]")[0];
		var circleSelect = gState.querySelectorAll("[class=circle_text]")[0];

		console.log(gState);
		console.log(state);
		console.log(circleSelect);

		info_manager.state_elements[gState.id] = 
			new StateElement( state );

		info_manager.state_elements[gState.id].input_transitions = 
			newStateElements[gState.id].input_transitions;
		info_manager.state_elements[gState.id].output_transitions = 
			newStateElements[gState.id].output_transitions;

		circleSelect.addEventListener("click", UpdateCurrentEl, false);
		gState.addEventListener("mouseenter", mouseEnterState, false);
		gState.addEventListener("mouseleave", mouseLeaveState, false);
		state.addEventListener("mousedown", clickEventEl, false);
	...
	for ( var i = 0; i < transitions.length; i++ ){
		...
		info_manager.transitions_elements[transition.id] = 
			new TransitionElement( transition );

		info_manager.transitions_elements[transition.id].orgState = 
			newTransitionsElements[transition.id].orgState;

		info_manager.transitions_elements[transition.id].dstState = 
			newTransitionsElements[transition.id].dstState;

		info_manager.transitions_elements[transition.id].info_transition = 
			newTransitionsElements[transition.id].info_transition;

		transition.addEventListener("click", Properties, false);

	}

	info_manager.svg = svg;
